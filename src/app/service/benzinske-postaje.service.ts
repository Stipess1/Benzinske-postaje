    ...
    this.storage.get('updateVrijeme').then((data: Date) => {
      ...
      if (diffDays >= 1 || data == null) {
        ...
        this.httpClient.get(heroku).subscribe((data: any) => {
          ...
          this.storage.set('postaje', JSON.stringify(this.svePostaje));
          this.storage.set('goriva', JSON.stringify(this.svaGoriva));
          this.storage.set('updateVrijeme', new Date())
        ...
        this.storage.get('postaje').then(postaje => {
          postaje = JSON.parse(postaje);
          
          
          for(let i = 0; i < postaje.length; i++) {
            let postaja = new Postaja();
            postaja.adresa = postaje[i]['adresa'];
            postaja.id = postaje[i]['id'];
            postaja.lat = postaje[i]['lat'];
            postaja.long = postaje[i]['long'];
            postaja.mjesto = postaje[i]['mjesto'];
            postaja.naziv = postaje[i]['naziv'];
            postaja.obveznikId = postaje[i]['obveznikId'];
            postaja.obveznik = postaje[i]['obveznik'];
            postaja.img = postaje[i]['img'];
            postaja.trenutnoRadnoVrijeme = postaje[i]['trenutnoRadnoVrijeme'];
            postaja.udaljenost = postaje[i]['udaljenost'];
            postaja.gorivo = postaje[i]['gorivo'];
            

            let radnoVrijeme = new RadnoVrijeme();
            radnoVrijeme.ponPet = postaje[i]['radnaVremena']['ponPet'];
            radnoVrijeme.sub = postaje[i]['radnaVremena']['sub'];
            radnoVrijeme.ned = postaje[i]['radnaVremena']['ned'];
            radnoVrijeme.praznik = postaje[i]['radnaVremena']['praznik'];

            postaja.radnaVremena = radnoVrijeme;

            for(let j = 0; j < postaje[i]['opcije'].length; j++) {
              postaja.opcije.push(new Usluge(postaje[i]['opcije'][j]['opcija_id']));
            }

            for(let j = 0; j < postaje[i]['cijenici'].length; j++) {
              let cijenik = new Cijenik();
              cijenik.id = postaje[i]['cijenici'][j]['id'];
              cijenik.gorivo_id = postaje[i]['cijenici'][j]['gorivo_id'];
              cijenik.cijena = postaje[i]['cijenici'][j]['cijena'];
              cijenik.naziv = postaje[i]['cijenici'][j]['naziv'];
              cijenik.vrstaGorivaId = postaje[i]['cijenici'][j]['vrstaGorivaId'];
              postaja.cijenici.push(cijenik);
            }
            this.getBenzinskaTimeFromJson(postaja.trenutnoRadnoVrijeme, postaja);
            this.svePostaje.push(postaja)
          }
          this.storage.get('goriva').then(goriva => {
            goriva = JSON.parse(goriva);
            for(let i = 0; i < goriva.length; i++) {
              let gorivo = new Gorivo();
              gorivo.id = goriva[i]['id'];
              if(goriva[i]['naziv'] != null)
                gorivo.naziv = goriva[i]['naziv'];
              gorivo.obveznik_id = goriva[i]['obveznik_id'];
              gorivo.vrstaGorivaId = goriva[i]['vrstaGorivaId'];
              this.svaGoriva.push(gorivo);
            }
